...
export default function StaffPanel({
  ...
  const adjustStock = async (itemId: string, delta: number) => {
    ...
    if (delta < 0) {
      updatedStats.productsSold = (updatedStats.productsSold || 0) + Math.abs(delta);
    }
    ...
    const updatedMenu = state.menu.map((item) => {
      if (item.id === itemId) {
        ...
        updatedStats.ordersLog = [logEntry, ...(updatedStats.ordersLog || [])].slice(0, 50);
    ...
    await onUpdateState({ menu: updatedMenu, stats: updatedStats });
  };
